...
stop_services() {
    echo -e "${CYAN}停止并禁用 sing-box 服务...${NC}"
...
clean_firewall_and_routes() {
    ...
    run_cmd ip rule del fwmark "$PROXY_FWMARK" table "$PROXY_ROUTE_TABLE"
...
remove_packages() {
    if confirm "是否卸载 sing-box 软件包? 默认不卸载 (y/N): "; then
        echo -e "${CYAN}正在卸载 sing-box 软件包...${NC}"
        run_cmd opkg remove sing-box
    fi
}
