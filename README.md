...
菜单中提供“卸载清理残留”功能，适合旧版本脚本残留导致菜单、网络设置或启动逻辑异常时使用。

该功能会停止并禁用 sing-box 服务、删除 `sb` 快捷命令、清理 `/etc/sing-box/scripts` 旧脚本、清理 sing-box 相关 nftables 和策略路由残留，并删除本脚本写入的转发持久化配置。

默认会保留 `/etc/sing-box/config.json`。删除现有配置、卸载 sing-box 软件包、删除 Ubuntu Netplan 静态网络配置都需要单独确认，避免误删配置或断网。
